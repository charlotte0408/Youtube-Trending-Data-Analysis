...
d24 <- dtm(docs)
wc24<- wordcloud(d24$word,d24$freq, type = 'text', colors=brewer.pal(8, "Dark2"), random.order = FALSE, rot.per=0.35, max.words = 200, scale=c(2.5, 0.15))
...
d10 <- dtm(docs10)
wc10<- wordcloud(d10$word,d10$freq, type = 'text', lang = "english", colors=brewer.pal(8, "Dark2"), random.order = FALSE, rot.per=0.35, max.words = 200, scale=c(2.5,0.15))
...
d26 <- dtm(docs26)
wc26<- wordcloud(d26$word,d26$freq, type = 'text', colors=brewer.pal(8, "Dark2"), random.order = FALSE, rot.per=0.3, max.words = 200, scale=c(3.5,0.25))

# category 23
docs23 <- preprocess(df_23)
v_rmwords <- c('commedy', "show", 'the', 'video')
docs23 <- text_cleaning(docs23, 'comedy')
d23 <- dtm(docs23)
